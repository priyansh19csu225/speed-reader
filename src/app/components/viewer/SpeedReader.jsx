...
class SpeedReader extends React.Component {
  ...
  componentWillReceiveProps = (nextProps) => {
    if (!this.props.isPlaying && nextProps.isPlaying) this.loop();

    if (
      this.props.setProgress &&
      this.props.setProgress.timestamp !== nextProps.setProgress.timestamp
    )
  ...
  loop = (opts) => {
    ...
    this.lastLoopId = setTimeout(() => {
      ...
      this.setState(
        Object.assign(this.getWordParts(currentText), {
          currentText,
      ...
      } else if (this.props.hasEndedCallback) this.props.hasEndedCallback();
    }, ms + this.offset);
  };
